	...
	<section class="container px-lg-5 my-0" style="margin-top:20px;">
			...
			<table style="width: 100%;">
				...
				<tbody>
					...
					<tr style="border-bottom-width: 10px; border-color: white;">
						...
						<td><a href="/WebContent/10_boardview.jsp?fbn=<%=list.get(i).getFbn()%>"><%=list.get(i).getFbo()%></a>
						</td>
						<!-- 링크걸어서 글내용 보이게 -->
		...
		<div class="fb-bottom-btn">
			...
			<button class="btn btn-outline-dark Previous">
				<a href="/WebContent/10_freeboard.jsp?pageNumber=<%=pageNumber - 1%>">이전페이지</a>
			</button>
			<%
				}
			...
			<button class="btn btn-outline-dark nextpage">
				<a href="/WebContent/10_freeboard.jsp?pageNumber=<%=pageNumber + 1%>">다음페이지</a>
			</button>
			<%
				}
			%>
			<button class="btn btn-outline-dark new-writh">
				<a href="/WebContent/11_boardwrite.jsp">글쓰기</a>
			</button>
		</div>
		<hr class="bar">
	</section>
